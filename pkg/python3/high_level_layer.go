...
// PyEval_EvalCode evaluates a Python code object.
func PyEval_EvalCode(code *PyObject, globals, locals *PyObject) *PyObject {
	cCode := (*C.PyObject)(unsafe.Pointer(code))
	cGlobals := (*C.PyObject)(unsafe.Pointer(globals))
	cLocals := (*C.PyObject)(unsafe.Pointer(locals))

	cResult := C.PyEval_EvalCode(cCode, cGlobals, cLocals)
	return togo(cResult)
}
